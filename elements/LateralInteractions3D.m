% LateralInteractions3D (COSIVINA toolbox)
%   Connective element that performs a convolution with a Mexican hat
%   kernel (difference of two Gaussians) with a global component over 
%   a three-dimensional space.
%
% Constructor call:
% LateralInteractions3D(label, size, sigmaExcY, sigmaExcX, sigmaZ,
%     amplitudeExc, sigmaInhX, sigmaInhY, sigmaInhZ, amplitudeInh, ...
%     amplitudeGlobal, circularY, circularX, circularZ, normalized, ...
%     cutoffFactor)
%   label - element label
%   size - size of input and output of the convolution (must be a
...
classdef LateralInteractions3D < Element
  ...
  properties (Constant)
    parameters = struct('size', ParameterStatus.Fixed, 'sigmaExcY', ParameterStatus.InitStepRequired, ...
      ...
      'circularZ', ParameterStatus.InitStepRequired, 'normalized', ParameterStatus.InitStepRequired, ...
      'cutoffFactor', ParameterStatus.InitStepRequired);
    components = {'kernelExcY', 'kernelExcX', 'kernelExcZ', 'kernelInhY', 'kernelInhX', 'kernelInhZ', ...
      'fullSum', 'output'};
  ...
  methods
    % constructor
    function obj = LateralInteractions3D(label, size, sigmaExcY, sigmaExcX, sigmaExcZ, amplitudeExc, ...
        sigmaInhY, sigmaInhX, sigmaInhZ, amplitudeInh, amplitudeGlobal, circularY, circularX, circularZ, ...
      ...
      if numel(obj.size) ~= 3
        error('LateralInteractions3D:Constructor:invalidArgument', 'Argument size must be a three-element vector.');
      end
    end
    ...
    function obj = step(obj, time, deltaT) %#ok<INUSD>
      ...
      obj.output = reshape(cnvExc - cnvInh, sz) + obj.amplitudeGlobal * obj.fullSum;
    end
