...
launcherOneLayerField and launcherOneLayerField_preset: These create the most basic DNF architecture, a single one-dimensional field with lateral interactions that receives external inputs. The GUI shows the neural field in the top plot and the interaction kernel in the bottom plot. Set inputs with the sliders to the bottom right, and change interactions patterns with sliders or by selecting a predefined pattern in the dropdown box (launcherOneLayerField_preset only; click Select to activate the chosen pattern). You can observe detection instability, selection between inputs, and self-sustained peaks in this model.
