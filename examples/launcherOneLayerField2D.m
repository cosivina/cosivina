...
% create interactions
sim.addElement(GaussKernel2D('u -> u (sharp)', [fieldSize, fieldSize], sigma_exc, sigma_exc, 0), ...
  'field u', [], 'field u');
sim.addElement(GaussKernel2D('u -> u (broad)', [fieldSize, fieldSize], sigma_inh, sigma_inh, 0), ...
  'field u', [], 'field u');
sim.addElement(SumDimension('u -> u (global)', [1, 2], [1, 1], 0), 'field u', [], 'field u');

% create noise stimulus and noise kernel
sim.addElement(NormalNoise('noise', [fieldSize, fieldSize], 1.0));
sim.addElement(GaussKernel2D('noise kernel', [fieldSize, fieldSize], 0, 0, 0), 'noise', [], 'field u');


%% setting up the GUI

elementGroups = {'field u', 'u -> u (sharp)', 'u -> u (broad)', 'u -> u (global)', ...
  'stimulus 1', 'stimulus 2', 'stimulus 3', 'stimulus 4', 'noise kernel'};

gui = StandardGUI(sim, [50, 50, 1000, 600], 0.01, [0.0, 0.3, 1.0, 0.7], [1, 2], 0.075, ...
  [0.0, 0.0, 1.0, 0.3], [6, 4], elementGroups, elementGroups);
...
gui.addControl(ParameterSlider('c_exc', 'u -> u (sharp)', 'amplitude', [0, 40], '%0.1f', 1, ...
  'strength of lateral excitation'), [2, 1]);
gui.addControl(ParameterSlider('c_inh', 'u -> u (broad)', 'amplitude', [0, 40], '%0.1f', -1, ...
  'strength of lateral inhibition'), [2, 2]);
gui.addControl(ParameterSlider('c_gi', 'u -> u (global)', 'amplitude', [0, 0.25], '%0.2f', -1, ...
...
gui.addControl(ParameterSlider('c_s4', 'stimulus 4', 'amplitude', [0, 20], '%0.1f', 1, ...
  'stength of stimulus 4'), [6, 3]);
