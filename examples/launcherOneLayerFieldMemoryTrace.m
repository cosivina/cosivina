% Launcher for a one-layer neural field simulator with a memory trace.
% The script sets up an architecture with a single one-dimensional neural
% field coupled to a memory trace, lateral interactions, and three Gaussian
% inputs to the field. It then creates a GUI for this architecture and
% starts that GUI. Hover over sliders and buttons to see a description of
% their function.
...
sim.addElement(MemoryTrace('mem u', fieldSize, 100, 1000, 0.5), 'field u', 'output');
...
elementGroups = {'field u', 'u -> u', 'mem u', 'mem u -> u', 'stimulus 1', 'stimulus 2', 'stimulus 3', ...
  'noise kernel'};
elementGroupLabels = {'field u', 'kernel u -> u', 'memory trace mt_u', 'kernel  mt_u -> u', ...
  'stimulus 1', 'stimulus 2', 'stimulus 3', 'noise kernel'};

gui = StandardGUI(sim, [50, 50, 1000, 750], 0.01, [0.0, 1/4, 1.0, 3/4], [2, 1], 0.06, [0.0, 0.0, 1.0, 1/4], [6, 4], ...
  elementGroupLabels, elementGroups);

gui.addVisualization(MultiPlot({'field u', 'field u', 'shifted stimulus sum', 'mem u'}, ...
  {'activation', 'output', 'output', 'output'}, ...
  [1, 10, 1, 10], 'horizontal', {'YLim', [-15, 15], 'XGrid', 'on', 'YGrid', 'on'}, ...
  {{'b', 'LineWidth', 3}, {'r', 'LineWidth', 2}, {'Color', [0, 0.75, 0], 'LineWidth', 2}, ...
  {'Color', 'c', 'LineWidth', 2} }, ...
  'field u (blue - activation, green - external input, red - output, cyan - memory trace)', ...
...
gui.addControl(ParameterSlider('c_glob', 'u -> u', 'amplitudeGlobal', [0, 2], '%0.1f', -1, ...
  'strength of global inhibition'), [2, 3]);
gui.addControl(ParameterSlider('c_mem', 'mem u -> u', 'amplitude', [0, 10], '%0.1f', 1, ...
  'strength of input from the memory trace to the field'), [3, 1]);
