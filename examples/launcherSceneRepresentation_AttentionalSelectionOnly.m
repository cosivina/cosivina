...
if nFeatures == 2
    ...
    gui = StandardGUI(sim, [25, 25, 1150, 900], 0.0, [0, 0.175, 1.0, 0.8], [4, 4], 0.015, [0, 0, 1.0, 0.15], [6, 4], ...
        elementGroups, elementsInGroup);
end
...
%         'ior s'), [1, 10], [1, 3]);
% else
%     gui.addVisualization(MultiPlot({'ior_s', 'ior_s', 'pd_c1', 'pd_c1', 'cos', 'cos'}, ...
...
for i = 1 : nFeatures
    ...
    ov = 2*i; % vertical offset for placing visualizations of the current feature
...
%         {{'Color', 'b'}, {'Color', 'r'}}, ['con f' n]), [ov, 8], [3, 1]);
    gui.addVisualization(MultiPlot({['atn_f' n], ['atn_f' n]}, {'activation', 'output'}, [1, 10], 'vertical', ...
        {'YLim', [1, fieldSize_ftr], 'XLim', [-15, 15], 'XDir', 'reverse', 'YAxisLocation', 'right', 'Box', 'on'}, ...
        {{'Color', 'b'}, {'Color', 'r'}}, ['atn f' n]), [ov, 1], [3, 1]);
...
gui.addControl(ParameterSwitchButton('spatial coupling ret<->allo', ...
    {'scale atn_sr -> atn_sa', 'scale atn_sa -> atn_sr', 'scale ior_s -> atn_sa', 'atn_sa', 'atn_c1'}, ...
    {'amplitude', 'amplitude', 'amplitude', 'h', 'h'}, [0.0, 0.0, 0.0, -3, -4.75], [1.0, 1.0, 1.0, -5, -5], ...
...
if nFeatures == 2
    ...
    gui.addControl(ParameterSwitchButton('stimulus pattern A', ...
        {'i1 for vis_f1', 'i2 for vis_f1', 'i3 for vis_f1'}, ...
        {'amplitude', 'amplitude', 'amplitude'}, zeros(1, 3), 6 * ones(1, 3), ...
        [], false), [1, 3]);
    gui.addControl(ParameterSwitchButton('stimulus pattern B', ...
        {'i4 for vis_f1', 'i5 for vis_f1', 'i6 for vis_f1'}, ...
        {'amplitude', 'amplitude', 'amplitude'}, zeros(1, 3), 6 * ones(1, 3), ...
        [], false), [2, 3]);
    gui.addControl(ParameterSwitchButton('stimulus pattern C', ...
        {'i3 for vis_f1', 'i1 for vis_f1', 'i2 for vis_f1'}, ...
        {'amplitude', 'amplitude', 'amplitude'}, zeros(1, 3), 6 * ones(1, 3), ...
        [], false), [3, 3]);
end
