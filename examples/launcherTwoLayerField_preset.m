% Launcher for a two-layer neural field simulator with parameter presets.
...
elementGroups = {'field u', 'field v', 'u -> u', 'u -> v', 'v -> u (local)', 'v -> u (global)', ...
  'stimulus 1', 'stimulus 2', 'stimulus 3', 'noise kernel u', 'noise kernel v'};

gui = StandardGUI(sim, [50, 50, 900, 750], 0.01, [0.0, 1/4, 1.0, 3/4], [2, 1], 0.06, [0.0, 0.0, 1.0, 1/4], [7, 4], ...
...
gui.addControl(ParameterSlider('uu', 'u -> u', 'amplitude', [0, 40], '%0.1f', 1, ...
  'strength of lateral excitation in field u'), [3, 1]);
gui.addControl(ParameterSlider('vu', 'u -> v', 'amplitude', [0, 40], '%0.1f', 1, ...
  'strength of excitation from field u to field v'), [3, 2]);
gui.addControl(ParameterSlider('uv_li', 'v -> u (local)', 'amplitude', [0, 40], '%0.1f', -1, ...
